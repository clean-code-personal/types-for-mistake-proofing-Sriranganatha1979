...
namespace brighteningtest
{
	...
	public:
		...
		TEST_METHOD(CheckForRowColumnMismatch)
		{
			...
			auto func = [image, this, &attenuatedPixelCount] {AddBrighteningImage(
				image, this->CreateImageWithDifferentMatrixSize(0, 25, 0, 25), attenuatedPixelCount); };
		...
		TEST_METHOD(BrightensWholeImage)
		{
			...
			Assert::AreEqual(90, int(brightenedImage->getPixelAtIndex(1, 0)));
		}
		...
		TEST_METHOD(BrightensWithAnotherImage)
		{
            ...
            Assert::AreEqual(45, int(brightenedImage->getPixelAtIndex(0, 0))); // left-side pixel is unchanged
            Assert::AreEqual(80, int(brightenedImage->getPixelAtIndex(0, 1))); // right-side pixel is brightened
